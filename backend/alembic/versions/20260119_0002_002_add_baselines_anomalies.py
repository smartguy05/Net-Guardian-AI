...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE anomalytype AS ENUM ('new_domain', 'volume_spike', 'time_anomaly', 'new_connection', 'new_port', 'blocked_spike', 'pattern_change');")
    op.execute("CREATE TYPE anomalystatus AS ENUM ('active', 'reviewed', 'false_positive', 'confirmed');")
    ...
    op.create_table(
        ...
        sa.Column("baseline_type", postgresql.ENUM("dns", "traffic", "connection", name="baselinetype", create_type=False), nullable=False),
        sa.Column("status", postgresql.ENUM("learning", "ready", "stale", name="baselinestatus", create_type=False), nullable=False, server_default="learning"),
    ...
    op.create_unique_constraint(
        "uq_device_baselines_device_type",
        "device_baselines",
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("device_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("devices.id", ondelete="CASCADE"), nullable=False),
        sa.Column("anomaly_type", postgresql.ENUM("new_domain", "volume_spike", "time_anomaly", "new_connection", "new_port", "blocked_spike", "pattern_change", name="anomalytype", create_type=False), nullable=False),
        ...
        sa.Column("status", postgresql.ENUM("active", "reviewed", "false_positive", "confirmed", name="anomalystatus", create_type=False), nullable=False, server_default="active"),
