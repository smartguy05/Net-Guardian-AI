...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE devicetype AS ENUM ('pc', 'mobile', 'iot', 'server', 'network', 'unknown');"
    )
    ...
    op.execute(
        "CREATE TYPE parsertype AS ENUM ('adguard', 'unifi', 'pfsense', 'json', 'syslog', 'nginx', 'custom');"
    )
    op.execute(
        "CREATE TYPE eventtype AS ENUM ('dns', 'firewall', 'auth', 'http', 'system', 'network', 'unknown');"
    )
    op.execute(
    ...
    op.create_table(
        ...
        sa.Column(
            "ip_addresses", postgresql.ARRAY(sa.String(45)), nullable=False, server_default="{}"
        ),
        ...
        sa.Column(
            "profile_tags", postgresql.ARRAY(sa.String(50)), nullable=False, server_default="{}"
        ),
        ...
        sa.Column(
            "status",
            postgresql.ENUM(
                "active", "inactive", "quarantined", name="devicestatus", create_type=False
            ),
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "source_type",
            postgresql.ENUM(
                "api_pull", "file_watch", "api_push", name="sourcetype", create_type=False
            ),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "source_id",
            sa.String(100),
            sa.ForeignKey("log_sources.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "event_type",
