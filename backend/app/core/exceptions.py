"""Custom exceptions for NetGuardian AI."""

from typing import Any


class NetGuardianError(Exception):
    """Base exception for all NetGuardian errors."""

    def __init__(
        self,
        message: str,
        details: dict[str, Any] | None = None,
        status_code: int = 500,
    ) -> None:
        self.message = message
        self.details = details or {}
        self.status_code = status_code
        super().__init__(self.message)


# Alias for backwards compatibility
NetGuardianException = NetGuardianError


class AuthenticationError(NetGuardianError):
    """Raised when authentication fails."""

    def __init__(
        self,
        message: str = "Authentication failed",
        details: dict[str, Any] | None = None,
    ) -> None:
        super().__init__(message, details, status_code=401)


class AuthorizationError(NetGuardianError):
    """Raised when user lacks required permissions."""

    def __init__(
        self,
        message: str = "Insufficient permissions",
        details: dict[str, Any] | None = None,
    ) -> None:
        super().__init__(message, details, status_code=403)


class NotFoundError(NetGuardianError):
    """Raised when a requested resource is not found."""

    def __init__(
        self,
        message: str = "Resource not found",
        details: dict[str, Any] | None = None,
    ) -> None:
        super().__init__(message, details, status_code=404)


class ValidationError(NetGuardianError):
    """Raised when input validation fails."""

    def __init__(
        self,
        message: str = "Validation failed",
        details: dict[str, Any] | None = None,
    ) -> None:
        super().__init__(message, details, status_code=422)


class DatabaseError(NetGuardianError):
    """Raised when a database operation fails."""

    def __init__(
        self,
        message: str = "Database operation failed",
        details: dict[str, Any] | None = None,
    ) -> None:
        super().__init__(message, details, status_code=500)


class ExternalServiceError(NetGuardianError):
    """Raised when an external service call fails."""

    def __init__(
        self,
        message: str = "External service error",
        details: dict[str, Any] | None = None,
    ) -> None:
        super().__init__(message, details, status_code=502)


class RateLimitError(NetGuardianError):
    """Raised when rate limit is exceeded."""

    def __init__(
        self,
        message: str = "Rate limit exceeded",
        details: dict[str, Any] | None = None,
    ) -> None:
        super().__init__(message, details, status_code=429)
